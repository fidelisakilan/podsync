...
from .sync import Aliases, Counters, clean_staging, diff, read_audio_meta, track_key
...
async def _network_phase(
    ...
    missing, stale = diff(songs, ipod_keys, aliases)
    ...
    if not dry_run:
        ...
        if missing:
            downloaded = await _download_missing(api, missing, counters, jobs)
        else:
            console.print("Nothing to download.")
    return missing, stale, downloaded


async def _download_missing(api, missing, counters: Counters, jobs: int) -> list[tuple]:
    dl = apple_music.build_downloader(api, STAGING_DIR / "music", STAGING_DIR)
    downloaded = []
    semaphore = asyncio.Semaphore(jobs)
    with _progress() as progress:
        task = progress.add_task(f"Downloading ({jobs} workers)", total=len(missing))

        async def download_one(song):
        ...
        await asyncio.gather(*(download_one(s) for s in missing))
    return downloaded
...
def main(dry_run: bool, relogin: bool, jobs: int) -> None:
    ...
    for tool in ("ffmpeg", "mp4decrypt"):
        if not shutil.which(tool):
            console.print(f"[yellow]Warning: {tool} not found in PATH[/yellow]")
    ...
    if downloaded or stale:
        ...
        console.print("iPod already in sync.")

    _summary(counters)

    if counters.total_errors == 0:
        clean_staging()
        if dev and device.eject(dev, console):
